...
static void rfid_control_task(void *pvParameter __attribute__((unused)))
{
	...
	while (xQueueReceive(rfid_queue_handle, &evt, portMAX_DELAY) == pdTRUE)
	{
		...
		switch (evt.cmd.get)
		{
			...
			case RFID_GET_RSSI:
				{
					RFID_UINT8_EVENT_SEND(RFID_RESPONSE_RSSI_MAX + evt.arg.uint8, as3933_get_rssi(evt.arg.uint8));
					break;
				}
			case RFID_GET_RSSI_1_3:
				{
					uint8_t rssi[3] = { 0, 0, 0 };
					as3933_read_bytes(10, rssi, evt.arg.uint8);
			...
			case RFID_EVENT_W_UP:
				{
					RFID_BOOL_EVENT_SEND(RFID_RESPONSE_EVENT_W_UP, evt.arg.boolean);
					break;
				}
			default:
		...
		switch (evt.cmd.set)
		{
			...
			case RFID_SWITCH:
				{
					if (evt.arg.uint8 >= RFID_MAX)
						evt.arg.uint8 = RFID_2;
					as3933_switch(as3933_drv_handle[evt.arg.uint8]);
					break;
				}
			case RFID_RESET:
				{
			...
			case RFID_SET_AGC:
				{
					as3933_set_disable_agc(evt.arg.boolean);
					break;
				}
			case RFID_SET_FREQ_TOLERANCE:
				{
					if (evt.arg.uint8 > 3)
						evt.arg.uint8 = 3;
					as3933_set_freq_tolerance(evt.arg.uint8);
					break;
				}
			case RFID_SET_HYSTERESIS:
				{
					if (evt.arg.uint8 > 3)
						evt.arg.uint8 = 3;
					as3933_set_comparator_hysteresis(evt.arg.uint8);
					break;
				}
			case RFID_SET_CORRELATOR_ENABLE:
				{
					as3933_set_patern_correlation(evt.arg.boolean);
					break;
				}
			case RFID_SET_DATA_MASK:
				{
					as3933_set_mask_data_before_wu(evt.arg.boolean);
					break;
				}
			case RFID_SET_GAIN_BOOST:
				{
					as3933_set_3db_gain_boost(evt.arg.boolean);
					break;
				}
			...
			case RFID_SET_ANTENA_DUMPER_ENABLE:
				{
					as3933_enable_antenna_damper(evt.arg.boolean);
					break;
				}
			case RFID_SET_ANTENA_DUMPER_VALUE:
				{
					if (evt.arg.uint8 > 3)
						evt.arg.uint8 = 3;
					as3933_set_antenna_damper(evt.arg.uint8);
					break;
				}
			...
			case RFID_SET_INTERNAL_CAPACITOR_3:
				{
					if (evt.arg.uint8 > 31)
						evt.arg.uint8 = 31;
					as3933_set_capacity(evt.cmd.set - RFID_SET_INTERNAL_CAPACITOR, evt.arg.uint8);
					break;
				}
			case RFID_SET_GAIN_REDUCTION:
				{
					as3933_set_gain_reduction(evt.arg.uint8);
					break;
				}
			case RFID_SET_CHANNEL_ACTIVE:
				{
					as3933_set_channel(evt.arg.uint8, true);
					break;
				}
			case RFID_SET_ALL_CHANNEL_ON_OFF:
				{
					as3933_set_channel(0, evt.arg.boolean);
					break;
				}
			case RFID_SET_ROUTE_FREQ_DAT_OFF:
				{
					as3933_route_res_freq_on_dat(0, evt.arg.boolean);
					break;
				}
			case RFID_SET_ROUTE_FREQ_DAT:
				{
					if (evt.arg.uint8 > 0 && evt.arg.uint8 < 4)
					{
						as3933_route_res_freq_on_dat(evt.arg.uint8, true);
					}
					break;
				}
			case RFID_SET_ROUTE_OSC_CLK_DAT:
				{
					as3933_route_clock_on_dat(evt.arg.boolean);
					break;
				}
			case RFID_SET_AUTO_TIMEOUT:
				{
					as3933_set_auto_time_out(evt.arg.uint8);
					break;
				}
			case RFID_SET_BAND:
				{
					as3933_band_select(evt.arg.uint8);
					break;
				}
			case RFID_SET_BITRATE:
				{
					as3933_set_bitrate(evt.arg.uint8);
					break;
				}
			default:
				break;
